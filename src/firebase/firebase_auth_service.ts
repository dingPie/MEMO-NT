...
export class FbAuth {
  ...
  // 구글로 로그인
  async loginWithGoogle(update?: (user: User | null) => void) {
  ...
  // 깃허브로 로그인
  async loginWithGithub(update?: (user: User | null) => void) {
  ...
  // 유저정보 변화 감지
  async onCheckUser(update?: (user: User | null) => void) {
  ...
  async getUserInfo(user?: User): Promise<IUserInfo> {
    ...
    return new Promise((resolve, rejects) => {
      getDoc(docRef).then(result => resolve(result.data() as IUserInfo));
    });
  }
