...
export const setTalkTag = (tag: ITag, expand?: "expand") => {
  ...
  else if (tag.id === "toBeDeleted")
    return <Icon icon={faClockFour} color="#FFFFFF" />;
...
// 메모로 메모태그찾기
export const getTagWithMemo = (tags: ITag[], editMemo: IMemo) =>
  tags.filter(v => v.id === editMemo.tagId)[0];

// 태그 이름으로 태그 존재여부 검사
export const getTagWithTagName = (tags: ITag[], tagName: string) => {
  // 아예 삭제예정은 반환하지 않는다.
